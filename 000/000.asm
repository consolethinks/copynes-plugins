;Mapper 000 dumper (WLA-DX assembler)
;with a write for bank init on a mulity
;16/01/2022 - by Consolethinks
...
;mapper sets mirroring -> 0
             ...
             jsr send_byte
;send PRG data
             lda #$80       ;prg size middle byte
             jsr send_byte
             lda #$00       ;prg size high byte (low is always $00)
             jsr send_byte
             lda #$01       ;send prg
             jsr send_byte
;dump PRG-ROM
             lda #$80       ;REMOVE THIS if you want to dump a normal 000 (tho it won't affect much)
             sta $8002
             jsr prg        ;dump bank
             
;send CHR data
             lda #$20       ;chr size middle byte
             jsr send_byte
