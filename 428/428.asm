;Mapper 428 dumper (WLA-DX assembler)
;02/03/2021 - by Consolethinks
;No size measuring, it can overdump (however most carts have the same size)
...
;send PRG data
             ...
             jsr send_byte
;init mapper
             lda #$80       ;initial setup for 16K banks
             sta $A001
             lda #$00
             sta $8000
             sta $8001
             lda #$00       ;first 16K bank
             sta $6001
...
prg_loop:
             ...
             sta $6001
             lda curr_bank
             cmp #$08       ;128K, if not over max bank -> loop
             bne prg_loop
;send CHR data
             lda #$00       ;chr size middle byte
             jsr send_byte
             lda #$01       ;chr size high byte
             jsr send_byte
             lda #$02       ;send chr
             jsr send_byte
;init mapper
...
chr_loop:
             ...
             lda curr_bank
             sta $6001      ;chr bank
             cmp #$08       ;64K, if not over max bank -> loop
             bne chr_loop
;finalize
