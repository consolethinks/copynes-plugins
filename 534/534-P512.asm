;Mapper 534 dumper (WLA-DX assembler)
;25/12/2020 - by Consolethinks
...
;init mapper
             ...
             sta $8000
             lda #$00
             sta $8001      ;select bank 0 for MMC3.reg[6]
             sta $6000
             sta $6001
             sta $6002      ;set first 256KiB bank
             sta curr_bank  ;store curr bank value (0)
             lda #$03
             sta $6003      ;NROM-256 mode
;dump PRG-ROM
prg_loop:
             jsr prg        ;dump bank
             inc curr_bank  ;set next bank
             lda curr_bank  ;setting outer 256KiB bank
             and #$08
             ror
             ror
             sta $6000
             lda #$06       ;setting inner 32KiB bank
             sta $8000
             lda curr_bank
             and #$07
             rol
             ...
             sta $8001
             lda curr_bank
             cmp #$10       ;if not over max bank -> loop
             bne prg_loop
;finalize
